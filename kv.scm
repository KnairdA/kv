...
(define (perform-help arguments)
  (let ((printer (make-key-value-pretty-printer 0 30)))
    (print (format #f "~{~&~A~}"
                   (map (lambda (pair) (printer (first pair) (second pair)))
                        (list (list "show [STORE [KEY]]"           "prints stores, keys of stores or values of keys")
                              (list "all  [STORE [KEY]]"           "prints key value pairs of every store, a single store or a single key")
                              (list "write STORE KEY VALUE"        "writes VALUE to KEY in STORE")
                              (list "delete STORE KEY"             "deletes KEY in STORE")
                              (list "rename STORE OLD-KEY NEW-KEY" "renames OLD-KEY of STORE to NEW-KEY")))))))
...
(define commands (list (make-command "help"   perform-help)
                       (make-command "show"   perform-show)
